...
helm repo update
helm install vault hashicorp/vault --create-namespace --namespace $VAULT_NAMESPACE --kubeconfig "/home/ubuntu/.kube/config" --set "global.externalVaultAddr=http://vault.sfeir.local:8200"
...
TOKEN_REVIEW_JWT=$(kubectl get secret "$SECRET_NAME" -n $VAULT_NAMESPACE --output='go-template={{ .data.token }}' --kubeconfig /home/ubuntu/.kube/config| base64 --decode)
